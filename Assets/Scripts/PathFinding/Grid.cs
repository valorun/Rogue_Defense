        ...
        public List<Node> GetNeighbours(Node node)
        {
            ...
            for (int x = -1; x <= 1; x++)
            {
                for (int y = -1; y <= 1; y++)
                {
					...
					if (checkX >= 0 && checkX < gridSizeX && checkY >= 0 && checkY < gridSizeY  && Mathf.Abs(x)+Mathf.Abs(y)<2)  //check if the neighbours are in the grid (not outside) and are not diagonals (equals to : ((x==-1 && y==0) || (x==0 && y==1) ||(x==0 && y==-1) ||(x==1 && y==0)) )
                    {
